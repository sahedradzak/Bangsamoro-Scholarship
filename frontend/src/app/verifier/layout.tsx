...
const navigation = [
    ...
    { name: "Verification Queue", href: "/verifier/requests", icon: ClipboardList },
    { name: "History", href: "/verifier/history", icon: History },
    { name: "Batch Upload", href: "/verifier/batch", icon: Upload },
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Link href="/verifier" className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-900 hidden sm:inline-block">
                            Bangsamoro <span className="text-emerald-600">Verify</span>
                        </span>
                    </Link>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    <div className="hidden md:flex items-center relative max-w-xs">
                        ...
                        <input
                            ...
                            className="pl-9 pr-4 py-1.5 bg-slate-100 border-transparent rounded-full text-sm focus:bg-white focus:ring-2 focus:ring-emerald-600 transition-all w-64"
                        />
                    </div>
                    ...
                    <Avatar className="h-8 w-8 ring-2 ring-emerald-100">
                        <AvatarImage src="/avatars/verifier.jpg" />
                        <AvatarFallback className="bg-emerald-600 text-white text-xs">VF</AvatarFallback>
                    </Avatar>
                </div>
            </header>
            ...
            <div className="flex flex-1 relative">
                ...
                <aside className="hidden lg:flex w-64 border-r border-slate-200 bg-white flex-col sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
                    <nav className="p-4 space-y-1">
                        {navigation.map((item) => {
                            const isActive = pathname === item.href || (item.href !== "/verifier" && pathname.startsWith(item.href));
                            return (
                                <Link
                                    key={item.name}
                                    href={item.href}
                    ...
                    <div className="mt-auto p-4 border-t border-slate-100">
                        <div className="bg-emerald-600 rounded-xl p-4 text-white">
                            <p className="text-xs font-semibold uppercase tracking-wider opacity-80 mb-2">Queue Status</p>
                            <div className="flex items-center justify-between text-xs">
                                <span>Pending</span>
                                <span className="font-bold">12</span>
                            </div>
                            <div className="mt-2 h-1 bg-emerald-500 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-300 w-[40%]"></div>
                            </div>
                        </div>
                    </div>
                </aside>
